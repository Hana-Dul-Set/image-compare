...
def get_image_size_and_sum(image_data):
    return image_data[1], image_data[2]
...
def is_same_image(image_data, target_image_data, IMAGE_DIR_PATH):
    image_filename = image_data[0]
    image_size, image_pixel_sum = get_image_size_and_sum(image_data)

    target_image_filename = target_image_data[0]
    target_image_size, target_image_pixel_sum = get_image_size_and_sum(target_image_data)
...
def write_data_by_row(row_data):
...
def write_same_image_pairs(IMAGE_DIR_PATH):
    ...
    for i in tqdm(range(image_pixel_data_list_len)):
        ...
        for j in range(i+1, image_pixel_data_list_len):
            ...
            if(is_same_image(image_data, target_image_data, IMAGE_DIR_PATH)):
                write_data_by_row(image_filename + ',' + target_image_filename + '\n')
